...
% i=0;
proj_num = 100;     % number of used projections
krok = 900/proj_num;

% Preparing space for projections
% Coordinate order: column (u), angle, row (v)
projections = zeros(350,proj_num,350);
for i = 1:krok:900
    cd(Particles_images);

    obr = imread((D1(i).name)); % You need to transpose the projections! Load (rows, columns).
    projections(:,(i+8)/9,:) = -obr;                       % See load proj_data in sample s006
end
IMG = im2double(projections,'indexed'); % Convert Image to Double Precision 


cd('C:\Users\Adil Al-Azzawi\Desktop\Particle Alignement');
% Parameters: #rows, #columns, 
vectors=100;
proj_geom = astra_create_proj_geom('cone_vec', 350, 350, vectors);
% The matrix must be of size (u,angles,v), where u is the number of columns of the detector
% and v the number of rows as defined in the projection geometry.
proj_id = astra_mex_data3d('create', '-proj3d', proj_geom, IMG);
